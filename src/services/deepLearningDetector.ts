...
export class DeepLearningDetector {
  ...
  private extractDeepFeatures(audio: ProcessedAudio): any {
    ...
    // 1. Spectrogram features (time-frequency representation)
    const spectrogram = this.computeSpectrogram(samples, sampleRate);
    
    // 2. Mel-spectrogram (perceptually relevant)
    const melSpectrogram = this.computeMelSpectrogram(samples, sampleRate);
    ...
    const chromaFeatures = this.computeChroma(samples, sampleRate);
  ...
  private analyzeMelPatterns(melSpectrogram: number[][]): number {
    ...
    if (energyDistribution < 0.3 || energyDistribution > 0.8) {
      aiScore += 0.4;
    }
